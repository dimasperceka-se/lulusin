...
const TRYOUT_TITLE = "Tryout CPNS HOTS 2026";
const TRYOUT_DESCRIPTION = "Latihan SKD HOTS (Higher-Order Thinking Skills) 2026: 30 TWK + 35 TIU + 45 TKP selama 100 menit. Disusun ulang & diparafrase dari materi CatBkn.com.";
const TRYOUT_DURATION_MINUTES = 100;
const TAG_PREFIX = "hots-2026:";

const REPO_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../..");
const XLSX_PATH = path.join(REPO_ROOT, "attached_assets/HOTS_2026.xlsx");
...
async function main() {
  ...
  console.log(`  IMPORT HOTS 2026 → "${TRYOUT_TITLE}" (package id=${CPNS_PACKAGE_ID})`);
  console.log("=".repeat(70));

  // Step 0: idempotent cleanup.
  console.log("\n[0/4] Cleanup previous hots-2026 import (if any)...");
  ...
  // Step 4: wire questions into tryout, preserving xlsx order (TWK → TIU → TKP)
